...
import { registerBorrower } from "../services/auth"
...
const BorrowerRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure address is properly formatted as expected by the backend
      // The backend expects address to be an object with specific fields
      if (!dataToSubmit.address.street || !dataToSubmit.address.city || !dataToSubmit.address.district) {
        setError("Address must include street, city, and district")
        setIsSubmitting(false)
        return
      }
