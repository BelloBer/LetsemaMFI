...
const LoanApplicationForm = () => {
  ...
  useEffect(() => {
    const fetchMFIs = async () => {
      setLoadingMFIs(true)
      try {
        // Use the correct API URL for fetching MFIs
        const response = await fetch(`${process.env.REACT_APP_API_URL}/mfis/active/`, {
          headers: {
            Authorization: `Bearer ${user.access}`,
            "Content-Type": "application/json",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL.replace("/users", "")}/loans/apply/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.access}`,
      ...
      if (!response.ok) {
        throw new Error(data.detail || "Failed to submit loan application")
      }
