...
// src/services/auth.js
const API_URL = process.env.REACT_APP_API_URL || "http://localhost:8000/api/users"
const BASE_API_URL = API_URL.replace("/users", "")
...
// Create API instance with auth token
export const createLoanApplication = async (token, loanData) => {
  const response = await fetch(`${BASE_API_URL}/loans/apply/`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
// Update the createAuthenticatedApi function to use the correct base URL
export const createAuthenticatedApi = (token) => {
  return {
    get: async (url) => {
      const response = await fetch(url, {
